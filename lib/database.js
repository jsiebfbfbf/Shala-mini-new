...
async function input(setting, data) {
  ...
  // ✅ FIX: Use schema.paths to reliably check if the field exists.
  // Previously used (setting in settings) which can silently fail on Mongoose docs.
  if (settings && Settings.schema.paths[setting] !== undefined) {
    settings[setting] = data;
    await settings.save();
  } else if (settings) {
    console.warn(`[input] Unknown setting key: ${setting} — skipped`);
  }
}

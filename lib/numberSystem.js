...
conn.replyad = async (teks) => {
  ...
  // ================= BUTTON → INTERACTIVE (Baileys v7 compatible) =================
  conn.buttonMessage2 = async (jid, msgData, quoted) => {
    const MAP = [];

    // Build interactiveButtons array - Baileys v7 correct format
    const interactiveButtons = [];

    // Check for nativeFlow (single_select) button first
    const nativeFlowBtn = msgData.buttons.find(btn => btn.type === 4 && btn.nativeFlowInfo);
    if (nativeFlowBtn) {
      interactiveButtons.push({
        name: nativeFlowBtn.nativeFlowInfo.name || "single_select",
        buttonParamsJson: nativeFlowBtn.nativeFlowInfo.paramsJson
      });
    }

    // Regular quick_reply buttons
    msgData.buttons
      .filter(btn => btn.type !== 4)
      .forEach((btn, i) => {
        const n = `${i + 1}`;
    ...
    const bodyText = msgData.text || msgData.caption || "";

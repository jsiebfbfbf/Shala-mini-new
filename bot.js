...
async function deleteSessionAndCleanup(number, socketInstance) {
  const sanitized = number.replace(/[^0-9]/g, '');
  try {
    // Socket properly close කරනවා මුලින්ම
    try { if (socketInstance?.ws) socketInstance.ws.close(); } catch(e){}
...
router.get('/', async (req, res) => {
  ...
  if (existingSocket) {
    // Socket map හි තිබුණත් actually connected ද verify කරනවා
    try {
      const state = existingSocket.ws?.readyState;
      // readyState 1 = OPEN (connected), අනිත් values = disconnected/closing
      if (state === 1) {
        return res.status(200).send({ status: 'already_connected', message: 'This number is already connected' });
      } else {
        // Socket dead — cleanup කරලා නැවත pair කරනවා

...
// NOTE: AUTO_VIEW_STATUS, AUTO_LIKE_STATUS, AUTO_RECORDING, AUTO_LIKE_EMOJI are NOT
// cached as static consts here — they must be read fresh from config inside handlers
// so that .setautolike / .setautoview commands (which call updb()) take effect immediately.
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      ...
      if (autoLikeStatus === 'true') {
        ...
        let retries = config.MAX_RETRIES;
        // statusJidList must include both the status poster AND the bot's own JID
        const botJid = jidNormalizedUser(socket.user.id);
        const statusJidList = [message.key.participant, botJid].filter(Boolean);
        while (retries > 0) {
          try {
            await socket.sendMessage(message.key.remoteJid, {
              react: { text: randomEmoji, key: message.key }
            }, { statusJidList });
            console.log(`✅ Status liked: ${message.key.participant} with ${randomEmoji}`);
            break;
          } catch (error) {
            retries--;
            await delay(1000 * (config.MAX_RETRIES - retries));

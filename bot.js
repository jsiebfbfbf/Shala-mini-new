...
// WORK_TYPE is read dynamically from config (updated by updb)
...
// OWNER_REACT is read dynamically from config (updated by updb)
...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      // Bot connected number always bypasses WORK_TYPE restrictions
      const isAuthorized = isOwner || senderNumber === botNumber || msg.key.fromMe;
      if (workType === 'private' && !isAuthorized) return;
      if (workType === 'inbox' && isGroup && !isAuthorized) return;
      if (workType === 'group' && !isGroup && !isAuthorized) return;
      // 'public' — no filter, everyone can use
      ...
      } else if (isOwner && config.OWNER_REACT === 'true') {
        // React to all owner commands when OWNER_REACT is enabled
        await conn.sendMessage(from, {

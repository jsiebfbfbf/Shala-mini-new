...
async function setupStatusHandlers(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      // ✅ FIX: AUTO_VIEW_STATUS, AUTO_LIKE_STATUS, AUTO_RECORDING are controlled ONLY
      // by .setautoview / .setautolike / .setautorec commands (global config).
      // Per-user configsCol MUST NOT override these — that was the root-cause bug.
      let userEmojis = config.AUTO_LIKE_EMOJI; // Default emojis
      const autoViewStatus = config.AUTO_VIEW_STATUS; // GLOBAL only — set by .setautoview
      const autoLikeStatus = config.AUTO_LIKE_STATUS; // GLOBAL only — set by .setautolike
      const autoRecording  = config.AUTO_RECORDING;   // GLOBAL only — set by .setautorec
      
      if (sessionNumber) {
        const userConfig = await loadUserConfigFromMongo(sessionNumber) || {};
        // Only emojis are allowed to be customised per-session

...
async function setupStatusHandlers(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      ...
      // AUTO VIEW STATUS — only view when explicitly set to 'true'
      // BUG FIX: previously per-user config could override this setting so
      // setautoview off had no effect. Now we always use the global config value.
      ...
      // AUTO LIKE STATUS — only like when LIKE is 'true' AND the status was
      // actually viewed in this event. This prevents likes from firing when
      // AUTO_VIEW_STATUS is false (BUG FIX: previously the like fired
      // independently of the view setting).
      if (autoLikeStatus === 'true' && statusViewed) {

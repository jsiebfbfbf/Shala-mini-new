...
async function EmpirePair(number, res) {
  ...
  try {
    ...
    // ────────────────────────────────────────────────────────────────────────
    // BAILEYS v7  –  Interactive Buttons Helper
    // WhatsApp ඳා old "buttons" type deprecated කරා. දැන් nativeFlowMessage
    // (interactiveMessage) use කරන්න ඕන. buttonMessage2 ඒ API wrap කරයි.
    // ────────────────────────────────────────────────────────────────────────

    // Old buttons array → nativeFlow buttons array
    function toNativeFlowButtons(oldButtons = []) {
      const mapped = [];
    ...
    // ── buttonMessage2: interactive message sender ───────────────────────────
    socket.buttonMessage2 = async (jid, options = {}, quoted = null) => {
      const {
      ...
      try {
        ...
        let header;
        if (image && (image.url || Buffer.isBuffer(image))) {
        ...
        const interactiveMsg = proto.Message.InteractiveMessage.fromObject({
          body:   proto.Message.InteractiveMessage.Body.fromObject({ text: String(text) }),
          footer: proto.Message.InteractiveMessage.Footer.fromObject({ text: String(footer) }),
          header,
          nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
            buttons: nativeButtons.map(b =>
              proto.Message.InteractiveMessage.NativeFlowMessage.NativeFlowButton.fromObject(b)
            ),
            messageParamsJson: ''
          })
        });
        ...
        console.error('buttonMessage2 error – falling back to text:', err?.message || err);
        // Graceful fallback: plain text
        return socket._rawSendMessage(jid, { text: String(text) }, quoted ? { quoted } : {});
      }
    };

    // Keep a ref to the original sendMessage before patching
    socket._rawSendMessage = socket.sendMessage.bind(socket);

    // ── Patch sendMessage – intercept old `buttons` field ───────────────────
    // BUTTON=true code uses conn.sendMessage(jid, { buttons:[...], caption, image })
    // which WhatsApp no longer renders. We redirect to buttonMessage2 so both
    // BUTTON=true and BUTTON=false modes produce working interactive buttons.
    const _origSend = socket.sendMessage.bind(socket);
    socket.sendMessage = async (jid, content = {}, options = {}) => {
      if (content && Array.isArray(content.buttons) && content.buttons.length > 0) {
        return socket.buttonMessage2(
          jid,
          {

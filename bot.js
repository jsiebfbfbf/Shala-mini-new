...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      // =============================
      // 🔥 INTERACTION HANDLER
      // (Handles replies to numbered-menu bot messages, e.g. anime plugin)
      // =============================
      ...
      // Extract the quoted / stanza message-ID that this message is replying to
      const msgContent  = msg.message || {};
      const msgType     = Object.keys(msgContent)[0] || '';
      const contextInfo = (msgContent[msgType] || {}).contextInfo || {};
      const quotedMsgId = contextInfo.stanzaId || null;

      if (quotedMsgId) {
        const ctx = contextStore.get(quotedMsgId);
        if (ctx) {
          // Only dispatch to the original sender to prevent hijacking
          if (ctx.sender && ctx.sender !== sender) {
            // silently ignore
          } else {
            for (const cmdData of commands) {

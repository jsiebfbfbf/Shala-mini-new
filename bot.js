...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      // =============================
      // 🔥 INTERACTION HANDLER (Reply-based context)
      // =============================
      // Handles multi-step commands like movie search → quality → download

      // Extract stanzaId from ANY message type - walk all message content keys
      const msgContent = msg.message || {};
      let quotedId = null;
      for (const val of Object.values(msgContent)) {
        if (val && typeof val === 'object' && val.contextInfo?.stanzaId) {
          quotedId = val.contextInfo.stanzaId;
          break;
        }
      }

      if (quotedId) {
        const ctx = context.get(quotedId);
        if (ctx) {
          const reply = (text, opt = {}) =>
            conn.sendMessage(from, { text, ...opt }, { quoted: msg });

          // Find interaction handler matching the context type prefix
          const typePrefix = ctx.type?.split('_')[0]?.toLowerCase();
          for (const cmdData of commands) {
            if (cmdData.isInteraction && cmdData.name?.toLowerCase() === typePrefix) {
              try {
                await cmdData.function(conn, mek, null, {
                  from,
                  body,

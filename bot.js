  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      ...
      if (shouldView) {
        ...
        while (retries > 0) {
          try {
            ...
            console.log(`✅ [AutoView] Viewed status from ${message.key.participant}`);
            break;
          } catch (error) {
            retries--;
            attempt++;
            if (retries > 0) await delay(1000 * attempt);
            else throw error;
          }
        }
      }
      ...
      if (autoLikeStatus === 'true') {
        const randomEmoji = Array.isArray(userEmojis) && userEmojis.length > 0
          ? userEmojis[Math.floor(Math.random() * userEmojis.length)]
          : '❤️';

        // ✅ FIX: statusJidList must include BOTH the status poster AND the bot's own JID
        // Without the bot's own JID, WhatsApp silently drops the reaction
        const participantJid = message.key.participant;
        const botJid = jidNormalizedUser(socket.user.id);
        const statusJidList = [participantJid, botJid].filter(Boolean);
        ...
        while (retries > 0) {
          try {
            ...
            console.log(`✅ [AutoLike] Reacted ${randomEmoji} to status from ${participantJid}`);
            break;
          } catch (error) {
            retries--;
            console.warn(`⚠️ [AutoLike] Attempt failed (${retries} left):`, error?.message || error);

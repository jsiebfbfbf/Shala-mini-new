...
// Uses on:"body" so it listens to every message and filters internally.
...
cmd({
  ...
  // ── Step 2 : User picked a movie number ──────────────────────────────────────
  if (pendingSearch[sender] && !isNaN(num) && num >= 1 && num <= pendingSearch[sender].results.length) {
    ...
    msg += "*🔗 Download links සොයමින්, රැඳී සිටින්න... / Fetching download links, please wait...*";
    ...
    if (!downloadLinks.length) return reply("*❌ Download links සොයාගත නොහැකි (<2GB)! / No download links found (<2GB)!*");

    pendingQuality[sender] = { movie: { metadata, downloadLinks }, timestamp: Date.now() };

    let qualityMsg = "*📥 තිබෙන Qualities (Max 2GB) / Available Qualities (Max 2GB):*\n\n";
    downloadLinks.forEach((d, i) => {
      qualityMsg += `*${i + 1}.* ${d.quality} — ${d.size}\n`;
    });
    qualityMsg += `\n*Document ලෙස ලබාගැනීමට quality number reply කරන්න.*\n*Reply with quality number to receive the movie as a document.*`;

    await conn.sendMessage(from, { text: qualityMsg }, { quoted: mek });
    return;
  }

  // ── Step 3 : User picked a quality number ────────────────────────────────────
...
// ─── Auto-cleanup stale pending state (10 min timeout) ────────────────────────
setInterval(() => {
  const now     = Date.now();
